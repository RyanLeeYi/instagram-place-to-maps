# Instagram / Threads Place → Google Maps 專案規格

## 專案概述

將 Instagram 和 Threads 內容（Reels 影片、圖片貼文、Threads 串文）中的餐廳/景點/店家資訊自動擷取，並同步到 Google Sheets，可連動 Google My Maps。

## 支援的內容類型

| 平台 | 類型 | URL 格式 | 處理方式 |
|------|------|----------|--------|
| Instagram | Reel | `instagram.com/reel/xxx` | 影片下載 → 語音轉文字 → 視覺分析 |
| Instagram | Reels | `instagram.com/reels/xxx` | 同上 |
| Instagram | 貼文（圖片）| `instagram.com/p/xxx` | 圖片下載 → 視覺分析 + 貼文說明 |
| Instagram | 貼文（影片）| `instagram.com/p/xxx` | 自動偵測並切換到影片流程 |
| Instagram | IGTV | `instagram.com/tv/xxx` | 影片下載 → 語音轉文字 → 視覺分析 |
| Instagram | 分享連結 | `instagram.com/share/xxx` | 自動偵測內容類型 |
| Threads | 貼文 | `threads.net/@user/post/xxx` | Googlebot UA 取得資料 → CDN 直接下載 |
| Threads | 貼文 | `threads.com/@user/post/xxx` | 同上 |
| Threads | 短連結 | `threads.net/t/xxx` | 同上 |
| Threads | 串文 | 同上（自動偵測） | 合併同作者的連續貼文 → 混合媒體分析 |
...
### 3. Threads 貼文分析
- 支援 threads.net 和 threads.com 連結
- 使用 Googlebot UA 觸發 Meta 伺服器端渲染，解析嵌入的 data-sjs JSON
- 直接從 CDN URL 下載媒體（不依賴 yt-dlp）
- 支援內容類型：圖片、影片、輪播、純文字
- **串文支援（thread chain）**：
  - 自動偵測並遍歷所有 `thread_items`
  - 過濾同一作者的貼文（排除其他人回覆）
  - 合併多篇 caption（`[1/N]`、`[2/N]` 編號 + 分隔線）
  - 聚合所有媒體 URL（圖片 + 影片）
  - 混合媒體串文會並行執行圖片分析和影片分析
...
│                    Telegram Bot                              │
│                  (接收 IG / Threads 連結)                      │
│         支援：Reel / 貼文 / IGTV / 分享連結 / Threads 貼文串文 │
└─────────────────────┬───────────────────────────────────────┘
                      │
                      ▼
┌─────────────────────────────────────────────────────────────┐
...
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
│  【影片流程】              │  【圖片貼文流程】    │  【Threads 流程】     │
│  ┌──────────────┐         │  ┌──────────────┐  │  ┌─────────────┐  │
│  │   Downloader │         │  │ Post Downloader│  │  │ Googlebot UA│  │
│  │   (yt-dlp)   │         │  │ (instaloader) │  │  │ + CDN 下載  │  │
│  └──────────────┘         │  └──────────────┘  │  └─────────────┘  │
│         │                  │         │                       │
│         ▼                  │         ▼                       │
│  ┌──────────────┐         │  ┌──────────────┐              │
...
- ✅ 自動偵測連結類型並選擇處理流程
- ✅ 接收 Threads 貼文連結（threads.net / threads.com）
- ✅ Threads 串文支援（自動合併同作者的連續貼文）
...
### Threads 處理
- ✅ 使用 Googlebot UA 觸發伺服器端渲染
- ✅ 解析 data-sjs JSON 取得貼文結構化資料
- ✅ 從 CDN 直接下載圖片/影片（不依賴 yt-dlp）
- ✅ 支援圖片、影片、輪播、純文字貼文
- ✅ 串文支援：遍歷 thread_items、同作者過濾、caption 合併、混合媒體並行分析
